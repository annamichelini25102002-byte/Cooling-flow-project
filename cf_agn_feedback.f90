...
real*8 :: dfe(N),zfe(N),dfestar(N),Ffe(N),tbv(N),twind,lwind,vwind, &
          ...
          alphatot,tst(N),tsn,t00,e00,ledd
integer :: sdr,Num,ncicli,isn,iagn,ncont,nsn,ivir,ncont2
character*7 nomefile,brifile,abundfile
real*8 :: TC, sigmaT, c_light, me
real*8, EXTERNAL :: Cool       !! radiative cooling subroutine (see below) !!
integer :: choice
real*8 :: snu
real*8 :: H_comp(N)
! --- AGN Feedback Parameters (Compton Heating) ---
TC = 2.0d8 ! Compton temperature (K), 2e8 K as suggested
sigmaT = 6.652d-25 ! Thompson cross section (cm^2)
c_light = 2.99792458d10 ! Speed of light (cm/s)
me = 9.1093837d-28 ! Electron mass (g)

! Pre-calculate the constant part of the Compton heating formula (A_comp)

A_comp = (4.d0 * sigmaT * kbol) / (2.d0 * me * c_light**2 * mu * mp)

!!Deciding snu
print*, 'SNU? 1=0.3, 2=0.85, 3=3'
read(*,*) choice
select case(choice)
    case(1)
        snu = 0.3
    ...
    case(3)
        snu = 3.0
    case default
        print*, 'not valid, using default value snu=0.3'
        snu = 0.3
end select
print*, 'snu = ', snu
...
! INITIAL CONDITIONS & PARAMETERS OF THE CLUSTER
 ...
 if(xb(i).ge.100.*cmkpc) rhost(i)=0.   !! stars truncated at 100 kpc
 ...
 mbh=1.d9*msun       !! Central BH mass
 ledd=1.3d46*(mbh/(1.d8*msun))   !! Eddington luminosity - use it later (maybe)
...
print*,'f_bar(rvir) = ',real(barfrac(ivir)) !! this is the baryon fraction at virial radius
 ...
 do i=2, N-1
    tcool(i)=max(e(i)/Cool(tem(i),d(i)),3.156e7)   !! cooling time: key quantity 
    gmed=0.5*(gtot(i)+gtot(i+1))
...
!! SUBSTEP 0: STELLAR TERMS, IRON DENSITY, SN ENERGY...
   ...
   tgyr=(t+tshift)/(1.e9*yr)   !! to simulate a stellar pop of initial age "tshift"
...
!! SUBSTEP 0.5: AGN HEATING (Compton)

! Calcola il tasso di riscaldamento Compton per unità di volume:
! H_comp(r) = rho(r) * (LAGN / (4*pi*r^2)) * A_comp * (TC - T(r))

do i=2, N-1
   
        H_comp(i) = d(i) * (ledd / (4.d0*pi*xb(i)**2)) * A_comp * (TC - tem(i))
  
    e(i) = e(i) + dtmin * H_comp(i)
...
!! Calculate the masses of hot gass, cold gas...
    ...
    do i=2,N-1
       ...
       if(tem(i).gt.tcold) mhotadd=d(i)*vol(i)
