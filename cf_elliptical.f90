...
! INITIAL CONDITIONS & PARAMETERS OF THE CLUSTER
 ...
 mvir=5e12*msun   !! virial mass of the cluster (includes DM and gas)
 mnfw=(1.-gasfrac)*mvir   !! Total dark matter mass (remove Mgas from Mvir)
 rvir=438*cmkpc  !! Virial radius
 rs=41.14*cmkpc    !! Scale radius for NFW halo (depends on mnfw!)
 conc=10.65       !! concentration of NFW halo
 fc=log(1.+conc)-conc/(1.+conc)

 mstars=6.e11*msun   !! Stellar mass of the BCG
 reff=8.*cmkpc      !! Effective radius
 ahern=reff/(1.+sqrt(2.))
...
do j=1,1000
    ...
    d0test = 0.5d0*(d0min + d0max)

    ! integrazione dell'equilibrio idrostatico
   lnd(2) = log(d0test)
    lnd(1) = log(d0test)
   do i=3,ivir
       lnd(i) = lnd(i-1) - (xb(i)-xb(i-1))*gtot(i)*(mu*mp)/(kbol*0.5*(tem(i)+tem(i-1))) &
  ...
  enddo
    ...
    if(abs(barfrac(ivir) - 0.16) < 1e-4) then
        den0 = d0test
        exit
    else if(barfrac(ivir) > 0.16) then
       d0max = d0test
    else
        d0min = d0test
    endif
...
den0 = d0test
print *, 'den0 e fb trovate =', den0, barfrac(ivir)
