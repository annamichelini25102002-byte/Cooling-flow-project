...
real*8 :: phi_b, phi_dm, phi_tot, Ekin, Eth, Etot, Epot, m_gas
...
open(20,file='grid_norad.dat')  !! here write the grid on file !!
do i=1,N
...
open(24,file='grv_norad.dat')     !! write some interesting stuff
open(25,file='mass_norad.dat')
do i=3,N
...
print*,'f_bar(rvir) = ',real(barfrac(ivir)) !! this is the baryon fraction at virial radius
 ...
 open(20,file='briiniz_norad.dat')   !briiniz= initial brightness ! in this file writes also tcool
 do i=2,ivir+1  !!N-1
    bri(i)=0.
    do nn=i,N-2
       epsilonx=0.
...
open(20,file='resiniz_norad.dat')
  do i=3,N
...
open(20,file='abundiniz_norad.dat')
  do i=2,N
...
!!!! Define some more parameters and start the time integration !!!!
 ...
 nomefile='cln0000' !file del cluster !cln0000= no radiative cooling
 brifile='brn0000' !file del brightness !brn0000= no radiative cooling
 abundfile='abn0000' !file di abundance

 open(22,file='mass_time_norad.dat')  !! writes the time evolution of masses
 open(23,file='mdot_time_norad.dat')  !! writes the time evolution of cooling rate
...
do i=2,N-1
   m_gas = d(i) * dvl1a(i)   ! massa del gas che evolve
   Ekin = Ekin + 0.5d0 * m_gas * v(i)**2 
   Eth  = Eth  + 1.5d0 * kbol * tem(i) * m_gas / (mu*mp)
!potenziale gravitazionale totale (DM+BCG)
   x = xb(i)/rs
   phi_dm = -grav * mvir/ (rs*fc) * dlog(1.d0+x)/x
   phi_b  = -grav * mstars / (xb(i) + a_hern)
   phi_tot = phi_dm + phi_b

   Epot = Epot + phi_tot * m_gas
  
end do
Etot=Ekin+Eth+Epot
open(50,file='energies.dat',status='unknown',position='append')
write(50, '(f12.5,1x,es16.8,1x,es16.8,1x,es16.8,1x,es16.8)') t/(1.e9*yr), Ekin, Eth, Epot, Etot
close(50)
...
!!!!  Here the hydro update ends  !!!!
 ...
 if(t.ge.(ncont+1)*deltat)then  !! write results every time interval deltat
    ncont=ncont+1
    call incnam(nomefile)   !! update name of the result datafile "cln0000**"
    open(20,file=nomefile)   !r,rr,densità,velocità,temperatura, pressione, tcool

    call incnam(brifile)    !! update name of the brightness datafile "brn0000**"
    open(42,file=brifile) !

    call incnam(abundfile)  !! update name of the abund datafile "abn0000**
